...
public class WorkOrderASImpl implements WorkOrderAS {
	...
	private static final String UPDATE_SERVICES_GOING = "update t_zot_work_order set service_time = ?, service_person_num = ?, "
			+ "service_object_id = ? where id = ?";
	...
	public void updateWorkOrderToGoing(XingWorkOrderBO workOrder) {
		JDBCTemplate<Object> sqlTemplate = new JDBCTemplate<Object>();
		List<Object> params = new ArrayList<Object>();	
		params.add(DateAS.getSQLTimestamp(workOrder.getService_time()));
		params.add(workOrder.getService_person_num());
		params.add(workOrder.getService_object_id());
		params.add(workOrder.getId());
		sqlTemplate.execute(UPDATE_SERVICES_GOING, params);
	}
